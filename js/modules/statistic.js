...
const ascDate = async () => {
    ...
    let postAcc = await fetch(`https://graph.instagram.com/me/media?fields=id, thumbnail_url, timestamp, media_url&access_token=${token}&limit=7&before=${before}`)
        .then(async response => await response.json().then(res => res))
        .catch(response => console.log(`terjadi error : ${response}`));
...
const moreAscDate = async () => {
    const morePost = await fetch(`https://graph.instagram.com/me/media?fields=id, thumbnail_url, timestamp, media_url&access_token=${token}&limit=8&before=${beforeAscDate}`)
        .then(async response => await response.json().then(res => res))
        .catch(response => console.log(`terjadi error : ${response}`));
